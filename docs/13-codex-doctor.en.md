...
|---|---|---|
| `doctor` | Full environment check (codex outdated-version detection, Windows execution policy, OneDrive pitfalls; `--no-network` skips probes, `--json` for scripts, `--strict` fails on warnings) | read-only |
