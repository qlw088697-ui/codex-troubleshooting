...
- **接续迭代优先看 [ROADMAP.md](ROADMAP.md)**：从待办里挑一项，按「实现 + 夹具测试 + 文档（中英同步）+ CHANGELOG + 版本号」一条龙做完，勾掉清单项；
- 一轮迭代一个主题，对应一个 `feat(vX.Y.Z)` commit；CHANGELOG 与 `package.json`、CLI 的 `VERSION` 三处同步升号。
